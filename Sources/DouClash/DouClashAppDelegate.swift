...
@MainActor
final class DouClashAppDelegate: NSObject, NSApplicationDelegate {
    private var statusItemController: DouClashStatusItemController?
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        statusItemController = DouClashStatusItemController()
    }
    ...
    private func runTerminationCleanupWithTimeout(seconds: TimeInterval) async {
        let cleanup = Task { @MainActor in
            await DouClashAppContext.shared.prepareForTermination()
        }
