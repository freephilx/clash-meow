...
enum DouClashProfileKind: String, Codable, Equatable {
    case local
    case remote
}

struct DouClashProfileSummary: Identifiable, Equatable {
    let id: String
    let name: String
    let fileURL: URL
...
struct ProfileRepository {
    ...
    static var defaultSubscriptionUserAgent: String {
        "DouClash/\(AppInfo.version) (clash.meta)"
    }
    ...
    func listProfiles() throws -> [DouClashProfileSummary] {
        try prepareStorage()
    ...
    @discardableResult
    func importLocalProfile(from url: URL) throws -> DouClashProfileSummary {
    ...
    @discardableResult
    func createBlankLocalProfile() throws -> DouClashProfileSummary {
        let name = "未命名"
        let id = stableProfileID(for: name)
        let destination = profileURL(for: id)
        try prepareStorage()
    ...
    @discardableResult
    func importRemoteProfile(from url: URL, useProxy: Bool, proxyPort: Int?) async throws -> DouClashProfileSummary {
    ...
    @discardableResult
    func refreshRemoteProfile(id: String, proxyPort: Int?) async throws -> DouClashProfileSummary {
        let metadata = try loadMetadata()
    ...
    private func sourceDescription(kind: DouClashProfileKind, remoteURL: URL?) -> String {
        switch kind {
        case .local:
            "Local YAML"
        case .remote:
    ...
    private func profileSort(_ left: DouClashProfileSummary, _ right: DouClashProfileSummary) -> Bool {
