...
struct RootView: View {
    ...
    var body: some View {
        ...
        .background(DouClashPalette.page)
    }
    ...
    private var sidebar: some View {
                    ...
                    ForEach(sidebarGroups) { group in
                        VStack(alignment: .leading, spacing: 4) {
                            Text(group.title)
                                ...
                                .foregroundStyle(DouClashPalette.dashboardSecondary)
        ...
        .background(DouClashPalette.sidebar)
...
private struct SidebarSettingsRow: View {
    ...
    var body: some View {
        Button {
            ...
            .foregroundStyle(DouClashPalette.ink)
            .padding(.horizontal, 10)
            .frame(height: 32)
...
private struct SidebarDestinationRow: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .foregroundStyle(isSelected ? DouClashPalette.accent : DouClashPalette.ink)
            .padding(.horizontal, 10)
            .frame(height: 32)
            .contentShape(RoundedRectangle(cornerRadius: 7, style: .continuous))
            .background(
                isSelected ? DouClashPalette.sidebarSelection : Color.clear,
    ...
    private var header: some View {
        HStack(spacing: 12) {
            VStack(alignment: .leading, spacing: 3) {
                Text("健康检查")
                    .font(.headline)
                Text("检查 DouClash、Mihomo 与当前网络的实际可用状态。")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
...
private enum HealthCheckState {
    ...
    var color: Color {
        switch self {
        case .ready: DouClashPalette.accent
        case .checking, .warning: DouClashPalette.orange
        case .failed: .red
    ...
    var body: some View {
        HStack(spacing: 8) {
            Image(systemName: "checkmark.circle.fill")
                ...
                .foregroundStyle(DouClashPalette.accent)
            Text(toast.message)
                .font(.system(size: 13, weight: .semibold))
                .foregroundStyle(DouClashPalette.ink)
        }
        ...
        .frame(height: 36)
        .background(DouClashPalette.card, in: Capsule())
        .overlay(
            Capsule()
                .stroke(DouClashPalette.faintLine, lineWidth: 1)
        )
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        HStack(spacing: 8) {
                            ...
                            Text("\(nodes.count)")
                                ...
                                .foregroundStyle(DouClashPalette.muted)
                                .padding(.horizontal, 8)
                                .padding(.vertical, 3)
...
private struct ProxyNodeSelectionDot: View {
    ...
    var body: some View {
        Circle()
            .fill(isSelected ? DouClashPalette.accent : DouClashPalette.faintLine)
...
private struct ProxyCard: View {
    ...
    var body: some View {
        ...
        Button(action: action) {
            ...
            .background(
                isSelected ? DouClashPalette.accent.opacity(0.10) : Color.white,
                in: RoundedRectangle(cornerRadius: 16, style: .continuous)
            )
            .overlay {
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .stroke(
                        isSelected ? DouClashPalette.accent.opacity(0.18) : DouClashPalette.faintLine.opacity(0.8),
                        lineWidth: 1
                    )
            }
...
private struct ProxyDelayStatusView: View {
    ...
    private static func presentation(for status: ProxyDelayStatus, referenceDate: Date) -> Presentation {
        ...
        case .timedOut(let previous, let attempt):
            ...
            return Presentation(
                text: "超时",
                color: isHistory ? DouClashPalette.muted : .red,
    ...
    private static func measurementPresentation(
        ...
        return Presentation(
            ...
            color: isStale ? DouClashPalette.muted : delayColor(measurement.milliseconds),
    ...
    private static func delayColor(_ delay: Int) -> Color {
        ...
        if delay < 500 { return DouClashPalette.orange }
        return .red
    }
...
private struct ConnectionRow: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            .buttonStyle(.plain)
            .foregroundStyle(DouClashPalette.orange)
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack(spacing: 8) {
                    TextField("搜索日志", text: $searchText)
                        .textFieldStyle(.douClashRounded)
...
private extension LogLevelFilter {
    var tintColor: Color {
        switch self {
        case .all: return DouClashPalette.muted
        case .error: return .red
        case .warning: return .orange
        case .info: return .blue
        case .debug: return DouClashPalette.muted
        }
    }
    ...
    var messageColor: Color {
        ...
        case .error, .warning:
            return tintColor
        case .all, .info:
            return DouClashPalette.ink
        case .debug:
            return DouClashPalette.muted
        }
    }
}
...
private extension LogSourceFilter {
    var tintColor: Color {
        switch self {
        case .all: return DouClashPalette.muted
        case .app: return DouClashPalette.accent
        case .core: return .indigo
        }
    }
}

private struct DouClashRoundedTextFieldStyle: TextFieldStyle {
    @Environment(\.isFocused) private var isFocused

    func _body(configuration: TextField<Self._Label>) -> some View {
        configuration
            .textFieldStyle(.plain)
            .tint(DouClashPalette.accent)
            .padding(.horizontal, 8)
            .padding(.vertical, 5)
            .background(DouClashPalette.card, in: RoundedRectangle(cornerRadius: 6, style: .continuous))
            .overlay {
                RoundedRectangle(cornerRadius: 6, style: .continuous)
                    .stroke(
                        isFocused ? DouClashPalette.accent.opacity(0.72) : DouClashPalette.faintLine,
                        lineWidth: 1
                    )
            }
    }
...
private extension TextFieldStyle where Self == DouClashRoundedTextFieldStyle {
    static var douClashRounded: DouClashRoundedTextFieldStyle {
        DouClashRoundedTextFieldStyle()
    }
}
...
private struct LogActionButton: View {
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                .font(.system(size: 12, weight: .semibold))
                .foregroundStyle(DouClashPalette.ink.opacity(0.76))
                .lineLimit(1)
                .padding(.horizontal, 14)
                .frame(height: 32)
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                ...
                .foregroundStyle(isSelected ? color : DouClashPalette.muted)
                .lineLimit(1)
                .padding(.horizontal, 12)
                .frame(width: width, height: height)
                .background(
                    (isSelected ? color : DouClashPalette.muted).opacity(0.09),
            ...
            HStack(spacing: 16) {
                if let proxyToggle = state.toggles.first(where: { $0.id == "proxy" }) {
                    FeatureCard(
                        ...
                        stateColor: proxyToggle.isOn ? DouClashPalette.accent : DouClashPalette.orange,
                ...
                if let allowLanToggle = state.toggles.first(where: { $0.id == "allowLan" }) {
                    FeatureCard(
                        ...
                        stateColor: state.allowLan ? DouClashPalette.accent : DouClashPalette.orange,
                        isOn: state.allowLan,
                        actionImage: nil
            ...
            HStack(spacing: 16) {
                if let tunToggle = state.toggles.first(where: { $0.id == "tun" }) {
                    FeatureCard(
                        ...
                        stateColor: state.isTunEnabled ? DouClashPalette.accent : DouClashPalette.orange,
            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                let featureCards = HStack(alignment: .top, spacing: 18) {
                    FeatureCard(
                        ...
                        stateColor: state.systemProxyEnabled ? DouClashPalette.accent : DouClashPalette.muted,
                    ...
                    FeatureCard(
                        ...
                        stateColor: state.isTunEnabled ? DouClashPalette.accent : DouClashPalette.muted,
                        isOn: state.isTunEnabled,
                        actionImage: nil,
                        isCompact: true,
                        minimumHeight: DashboardLayout.cardMinimumHeight,
...
private struct ProfilesContent: View {
    ...
    private var displayedProfiles: [DouClashProfileSummary] {
        let profileByID = Dictionary(uniqueKeysWithValues: state.profiles.map { ($0.id, $0) })
        var usedIDs = Set<String>()
        var profiles = profileDisplayOrder.compactMap { id -> DouClashProfileSummary? in
    ...
    private func profilesWithCurrentFirst(_ profiles: [DouClashProfileSummary]) -> [DouClashProfileSummary] {
    ...
    private var subscriptionURLField: some View {
        ...
        .overlay {
            RoundedRectangle(cornerRadius: 6, style: .continuous)
                .stroke(urlFieldFocused ? DouClashPalette.accent : DouClashPalette.faintLine, lineWidth: urlFieldFocused ? 2 : 1)
        }
    }
    ...
    var body: some View {
        ZStack {
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(DouClashPalette.accent.opacity(0.08))
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .stroke(
                    DouClashPalette.accent,
...
private struct ProfilesListRow: View {
    let profile: DouClashProfileSummary
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(alignment: .top, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text(profile.name)
                        .font(.system(size: 15, weight: .bold))
                        .foregroundStyle(profile.isCurrent ? DouClashPalette.accent : DouClashPalette.ink)
                        .lineLimit(1)
        ...
        .padding(16)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(DouClashPalette.card, in: RoundedRectangle(cornerRadius: 8, style: .continuous))
        .overlay {
...
private struct ProfileKindBadge: View {
    ...
    var body: some View {
        Text(text)
            .font(.system(size: 10, weight: .bold))
            .foregroundStyle(isSelected ? Color.white : DouClashPalette.muted)
            .padding(.horizontal, 7)
            .frame(height: 22)
            .background(
                isSelected ? DouClashPalette.accent : DouClashPalette.page,
...
private struct CorePowerSwitch: View {
    ...
    private var statusColor: Color {
        ...
        case .running:
            DouClashPalette.accent
        case .starting:
            DouClashPalette.orange
        case .failed, .missingBinary:
            Color.red
        case .stopped:
            DouClashPalette.dashboardSecondary
        }
    }
...
private struct GradientProgressBar: View {
    ...
    private var colors: [Color] {
        if isSelected {
            return [DouClashPalette.accent, DouClashPalette.accent.opacity(0.72)]
        }
    ...
    var body: some View {
        Button(action: action) {
            HStack(spacing: 12) {
                Text(label)
                    .font(.system(size: 11, weight: .semibold, design: .monospaced))
                    .foregroundStyle(selected ? DouClashPalette.accent : DouClashPalette.dashboardSecondary)
                    .frame(width: 34, height: 26)
                    .background((selected ? DouClashPalette.accent : DouClashPalette.dashboardSecondary).opacity(0.09), in: RoundedRectangle(cornerRadius: 6, style: .continuous))
                ...
                Circle()
                    .fill(selected ? DouClashPalette.accent : DouClashPalette.faintLine)
                    .frame(width: 7, height: 7)
            }
            .contentShape(Rectangle())
        }
                        ...
                        Button {
                            ...
                            HStack(spacing: 12) {
                                ...
                                Text(item.delayText ?? "--")
                                    ...
                                    .monospacedDigit()
                                    .foregroundStyle(item.alive == false ? DouClashPalette.orange : DouClashPalette.dashboardSecondary)
                                    .frame(width: 52, alignment: .trailing)
                                Circle()
                                    .fill(item.isSelected ? DouClashPalette.accent : DouClashPalette.faintLine)
                                    .frame(width: 7, height: 7)
                            }
                            .contentShape(Rectangle())
                        }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: isCompact ? 12 : 17) {
            ...
            HStack {
                ...
                Text(stateText)
                    .font(.system(size: 12, weight: .semibold))
                    .foregroundStyle(isOn ? DouClashPalette.accent : DouClashPalette.muted)
                Spacer()
                if let actionImage {
    ...
    var body: some View {
        Button(action: action) {
            ZStack(alignment: isOn ? .trailing : .leading) {
                Capsule()
                    .fill(isOn ? DouClashPalette.accent : DouClashPalette.faintLine)
            ...
            HStack(alignment: .top, spacing: 18) {
                VStack(spacing: 20) {
                    SummaryCard(
                        ...
                        statusColor: state.core.status.isHealthy ? DouClashPalette.accent : DouClashPalette.orange
                    )
                    TotalTrafficCard()
                }
                .frame(maxWidth: .infinity)
...
private struct InternetLatencyDiagnosticsSheet: View {
    ...
    private func color(for level: InternetDiagnosticEntry.Level) -> Color {
        ...
        case .warning: return DouClashPalette.orange
        case .info: return DouClashPalette.muted
        }
    }
}
...
private struct TotalTrafficCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(spacing: 5) {
                Capsule().fill(DouClashPalette.accent.opacity(0.35)).frame(maxWidth: .infinity)
                Capsule().fill(DouClashPalette.accent.opacity(0.35)).frame(maxWidth: .infinity)
            }
            .frame(height: 9)
        }
...
private struct TrafficListCard: View {
    ...
    private var rows: [(String, String, Color)] {
        let liveRows = state.activityTrafficRows.prefix(5).map { row in
            (row.name, formatByteCount(row.bytes), DouClashPalette.accent)
        }
        if !liveRows.isEmpty {
        ...
        HStack(alignment: .bottom, spacing: 12) {
            ForEach(Array(bars.enumerated()), id: \.offset) { _, bar in
                RoundedRectangle(cornerRadius: 2, style: .continuous)
                    .fill(DouClashPalette.accent.opacity(displayedSamples.isEmpty ? 0.25 : 1))
...
private extension View {
    ...
    func surfaceCard(cornerRadius: CGFloat = 16) -> some View {
        self
            .background(DouClashPalette.card, in: RoundedRectangle(cornerRadius: cornerRadius, style: .continuous))
            .overlay(
