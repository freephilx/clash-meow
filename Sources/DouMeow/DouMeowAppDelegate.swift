...
@MainActor
final class DouMeowAppDelegate: NSObject, NSApplicationDelegate {
    private var statusItemController: DouMeowStatusItemController?
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        statusItemController = DouMeowStatusItemController()
    }
    ...
    private func runTerminationCleanupWithTimeout(seconds: TimeInterval) async {
        let cleanup = Task { @MainActor in
            await DouMeowAppContext.shared.prepareForTermination()
        }
