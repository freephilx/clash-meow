...
enum NetworkDiagnosticReportService {
    ...
    static func retainRecentReports(in reportsRootURL: URL) throws {
        ...
        .filter {
            $0.pathExtension.lowercased() == "zip"
                && $0.deletingPathExtension().lastPathComponent.hasPrefix("DouMeow-Network-Diagnostics-")
                && isSafeRegularFile($0)
        }
        .sorted { $0.lastPathComponent > $1.lastPathComponent }
    ...
    static func reportBaseName(for date: Date) -> String {
        ...
        return "DouMeow-Network-Diagnostics-\(formatter.string(from: date))"
    }
    ...
    private static func runCommand(
        ...
        let outputURL = FileManager.default.temporaryDirectory
            .appending(path: "dou-meow-diagnostic-command-\(UUID().uuidString).log")
