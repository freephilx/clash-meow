...
enum DouMeowProfileKind: String, Codable, Equatable {
    case local
    case remote
}

struct DouMeowProfileSummary: Identifiable, Equatable {
    let id: String
    let name: String
    let fileURL: URL
...
struct ProfileRepository {
    ...
    static var defaultSubscriptionUserAgent: String {
        "DouMeow/\(AppInfo.version) (clash.meta)"
    }
    ...
    func listProfiles() throws -> [DouMeowProfileSummary] {
        try prepareStorage()
    ...
    @discardableResult
    func importLocalProfile(from url: URL) throws -> DouMeowProfileSummary {
    ...
    @discardableResult
    func createBlankLocalProfile() throws -> DouMeowProfileSummary {
        let name = "未命名"
        let id = stableProfileID(for: name)
        let destination = profileURL(for: id)
        try prepareStorage()
    ...
    @discardableResult
    func importRemoteProfile(from url: URL, useProxy: Bool, proxyPort: Int?) async throws -> DouMeowProfileSummary {
    ...
    @discardableResult
    func refreshRemoteProfile(id: String, proxyPort: Int?) async throws -> DouMeowProfileSummary {
        let metadata = try loadMetadata()
    ...
    private func sourceDescription(kind: DouMeowProfileKind, remoteURL: URL?) -> String {
        switch kind {
        case .local:
            "Local YAML"
        case .remote:
    ...
    private func profileSort(_ left: DouMeowProfileSummary, _ right: DouMeowProfileSummary) -> Bool {
