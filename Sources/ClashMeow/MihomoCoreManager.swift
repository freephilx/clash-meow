...
final class MihomoCoreManager: ObservableObject {
    ...
    func start() {
        ...
        if let validationError = validateRuntimeConfiguration(executablePath: launchPath) {
            status = .failed(validationError)
            AppLogSupport.error(validationError, module: "Core", logsDirectory: logsDirectory)
            return
        }
    ...
    private func copyBundledGeoDataIfNeeded() throws {
        guard let geoDataDirectory = Bundle.main.resourceURL?
            .appending(path: "Mihomo", directoryHint: .isDirectory)
            .appending(path: "GeoData", directoryHint: .isDirectory)
        else { return }

        for fileName in ["geoip.metadb", "Country.mmdb"] {
            let source = geoDataDirectory.appending(path: fileName)
            guard FileManager.default.fileExists(atPath: source.path) else { continue }
            let destination = configDirectory.appending(path: fileName)
            guard !FileManager.default.fileExists(atPath: destination.path) else { continue }
            try FileManager.default.copyItem(at: source, to: destination)
        }
    }

    private func validateRuntimeConfiguration(executablePath: String) -> String? {
        let task = Process()
        let arguments = ["-t", "-d", configDirectory.path, "-f", configFile.path]
        let outputURL = FileManager.default.temporaryDirectory
            .appending(path: "clash-meow-mihomo-test-\(UUID().uuidString).log")
        task.executableURL = URL(fileURLWithPath: executablePath)
        task.arguments = arguments
        task.currentDirectoryURL = configDirectory

        AppLogSupport.info(
            "校验 mihomo 配置: \(shellCommand(executable: executablePath, arguments: arguments))",
            module: "Core",
        ...
        do {
            ...
            let details = (try? String(contentsOf: outputURL, encoding: .utf8))?
                .trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            guard task.terminationStatus == 0 else {
                let reason = details.isEmpty ? "退出代码 \(task.terminationStatus)" : details
                return "mihomo 配置校验失败: \(reason)"
            }
        } catch {
            return "mihomo 配置校验无法执行: \(error.localizedDescription)"
        }
        AppLogSupport.info("mihomo 配置校验通过", module: "Core", logsDirectory: logsDirectory)
        return nil
    }
