...
struct NetworkDiagnosticReportTests {
    @Test @MainActor func reportGenerationCreatesPrivateZipAndCompletesProgress() async throws {
        let root = FileManager.default.temporaryDirectory
            .appending(path: "dou-meow-diagnostic-generate-\(UUID().uuidString)", directoryHint: .isDirectory)
        ...
        #expect(archive.lastPathComponent.hasPrefix("DouMeow-Network-Diagnostics-"))
    ...
    @Test func redactionRemovesCredentialsAndHomeDirectory() {
        ...
        \(home)/.config/dou-meow/runtime/logs/app.log
        ...
        #expect(output.contains("~/.config/dou-meow"))
    }

    @Test func recentLogsUseOnlyCurrentAndThreeNewestSafeArchives() throws {
        let directory = FileManager.default.temporaryDirectory
            .appending(path: "dou-meow-diagnostic-logs-\(UUID().uuidString)", directoryHint: .isDirectory)
    ...
    @Test func reportRetentionKeepsNewestFiveArchives() throws {
        let directory = FileManager.default.temporaryDirectory
            .appending(path: "dou-meow-diagnostic-retention-\(UUID().uuidString)", directoryHint: .isDirectory)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }

        for index in 1...7 {
            let archive = directory.appending(path: "DouMeow-Network-Diagnostics-20260809-18000\(index)-000.zip")
        ...
        #expect(names.filter { $0.hasPrefix("DouMeow-Network-Diagnostics-") }.count == 5)
        #expect(!names.contains("DouMeow-Network-Diagnostics-20260809-180001-000.zip"))
        #expect(!names.contains("DouMeow-Network-Diagnostics-20260809-180002-000.zip"))
